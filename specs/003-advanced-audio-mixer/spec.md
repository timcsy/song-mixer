...
- Q: 後端混音產生時的任務佇列處理方式？ → A: 複用現有 Job 佇列（與音軌分離任務相同機制）
- Q: 音軌音量預設值設定？ → A: 人聲 0%，其他音軌 100%（預設去人聲）
- Q: 升降 Key 後的音質處理優先級？ → A: 維持原速優先（音高改變但速度不變）
- Q: 混音檔案的快取策略？ → A: 快取相同設定的結果（設定相同則複用）
- Q: 前端音軌載入策略？ → A: 漸進式載入（串流播放，邊載邊播）
...
- **FR-011**: 系統 MUST 使用現有 Job 佇列機制處理混音產生任務
- **FR-012**: 系統 MUST 在預覽頁面預設人聲音量為 0%，其他音軌為 100%
- **FR-013**: 系統 MUST 在升降 Key 時維持原始播放速度（僅改變音高）
- **FR-014**: 系統 SHOULD 快取相同混音設定的輸出檔案以供複用
- **FR-015**: 系統 SHOULD 支援音軌漸進式載入，允許邊載入邊播放
...
- **MixJob（混音任務）**: 複用現有 Job 佇列機制，追蹤混音產生進度與狀態
- **MixCache（混音快取）**: 儲存已產生的混音檔案，以設定雜湊值作為索引
